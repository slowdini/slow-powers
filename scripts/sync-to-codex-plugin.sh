...
# (including committed Codex files under codex/ and assets/), preserves
...
[[ -f "$UPSTREAM/codex/plugin.json" ]] || die "committed Codex manifest missing at $UPSTREAM/codex/plugin.json"

# Read the upstream version from the committed Codex manifest.
UPSTREAM_VERSION="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["version"])' "$UPSTREAM/codex/plugin.json")"
...
Creates \`plugins/superpowers/\` by copying the tracked plugin files from upstream, including \`codex/plugin.json\` and \`assets/\`.
