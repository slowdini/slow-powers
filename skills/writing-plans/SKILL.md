...
This skill applies **once you're committed to writing a plan**. It does not push you into planning when the user wants direct action. If the user said "just build it" or "go fix it", trust the intent and skip this skill.
...
* You're updating an existing plan file (`implementation_plan.md`, `task.md`, or equivalent).
...
* You're tempted to draft a plan "to be safe" when the user is asking for execution.
...
3. **Show, Don't Describe.** If a step changes code, include the code. If a step runs a command, include the command and the expected outcome. "Similar to Task 3" is forbidden — repeat the content; the engineer may read tasks out of order.
...
| "Repeating code in similar tasks is wasteful." | The engineer may read tasks out of order. Repeat the code. |
