...
A run ends with teardown (`bun run evals:teardown --skill <name>`, or the `teardown` runner command): it disarms the guard **and** removes the staged skill set the runner created under `<cwd>/.claude/skills/`, so a completed run leaves nothing staged behind. Pre-existing project skills and `.claude/settings.json` are left intact.
