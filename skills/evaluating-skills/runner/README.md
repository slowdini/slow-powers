...
- **Internal use** points `--skill-dir` at the repo's `./skills`, so the skill-under-test sees every other superslow skill as a sibling — the realistic install. The npm scripts bake this in (`--skill-dir ./skills`), so maintainers keep using `bun run evals -- --skill <name> --mode <mode>` unchanged.
- **A user evaluating one personal skill** points `--skill-dir` at the directory holding it. If that directory contains only their skill, the eval runs in isolation — no sibling skills are staged. To include superslow skills as siblings, the user copies or symlinks them into `--skill-dir`.
