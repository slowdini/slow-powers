...
type PlannerToolCall = { name: string; args?: unknown };
...
function parseArgValue(val: unknown): unknown {
  ...
  try {
    ...
    if (!Number.isNaN(Number(cleanedStr)) && cleanedStr !== "")
...
export function listSubagents(subagentsDir: string): SubagentEntry[] {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const pendingSubagents: { subagents: RawSubagent[] }[] = [];
          ...
          if (record.type === "PLANNER_RESPONSE") {
            ...
            for (const tc of toolCalls) {
              if (tc.name === "invoke_subagent") {
                ...
                const subagents =
                  (args as { Subagents?: RawSubagent[] }).Subagents || [];
            ...
            if (nextPending) {
              ...
              const uuids: string[] = [];
              let match: RegExpExecArray | null = uuidRegex.exec(content);
              while (match !== null) {
                uuids.push(match[1]);
                match = uuidRegex.exec(content);
              }
              if (uuids.length > 0) {
                for (
                  let k = 0;
                  ...
                  const desc =
                    subagent.Role || subagent.Prompt || subagent.TypeName || "";
